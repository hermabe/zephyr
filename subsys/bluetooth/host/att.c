...
static void info_from_att_chan(const struct bt_att_chan *att_chan, struct bt_att_chan_info *info)
{
	(void)memset(info, 0, sizeof(*info));
	info->conn = att_chan->att->conn;
	info->cid = att_chan->chan.tx.cid;
	info->mtu = att_chan->chan.tx.mtu;
	info->mps = att_chan->chan.tx.mps;
}

static void att_chan_connected(struct bt_att_chan *att_chan)
{
	struct bt_eatt_cb *cb;
	struct bt_att_chan_info info;
	...
	info_from_att_chan(att_chan, &info);

	SYS_SLIST_FOR_EACH_CONTAINER(&callback_list, cb, node) {
		if (cb->eatt_chan_connected) {
			cb->eatt_chan_connected(&info);
		}
	}
}

...
struct net_buf *bt_att_chan_create_pdu(struct bt_att_chan *chan, uint8_t op, size_t len)
{
	struct bt_att_hdr *hdr;
	struct net_buf *buf;

	if (len + sizeof(op) > chan->chan.tx.mtu) {
		BT_WARN("ATT MTU exceeded, max %u, wanted %zu", chan->chan.tx.mtu,
			len + sizeof(op));
		return NULL;
	}
...
static int bt_att_chan_send(struct bt_att_chan *chan, struct net_buf *buf, bt_att_chan_sent_t cb)
{
...
static k_timeout_t credit_based_connection_delay(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) && conn->role == BT_CONN_ROLE_CENTRAL) {
		...
		const uint32_t rand_delay = random & 0x7; /* Small random delay for IOP */

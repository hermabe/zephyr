...
struct bt_att {
	...
	struct {
		...
		uint16_t prev_eatt_conn_rsp_result;
		uint16_t prev_eatt_conn_req_result;
		uint8_t prev_eatt_conn_req_missing_chans;
	} conn_context;
#endif /* CONFIG_BT_EATT */
};
...
static void att_enhanced_connection_work_handler(struct k_work *work)
{
	...
	const struct bt_att *att = CONTAINER_OF(dwork, struct bt_att, conn_context.connection_work);
	const int err = bt_eatt_connect(att->conn, att->conn_context.eatt_chans_to_connect);

	if (err < 0) {
		BT_WARN("Failed to connect %d EATT channels (err: %d)",
			att->conn_context.eatt_chans_to_connect, err);
	}
...
#if defined(CONFIG_BT_EATT)
	k_work_init_delayable(&att->conn_context.connection_work, att_enhanced_connection_work_handler);
#endif /* CONFIG_BT_EATT */
...
static int att_schedule_eatt_connect(struct bt_conn *conn, uint8_t eatt_chans_to_connect)
{
	...
	att->conn_context.eatt_chans_to_connect = eatt_chans_to_connect;

	return k_work_reschedule(&att->conn_context.connection_work,
...
{
	int err;
	size_t to_connect = att->conn_context.prev_eatt_conn_req_missing_chans;
	...
	if (att->conn_context.prev_eatt_conn_rsp_result == BT_L2CAP_LE_ERR_NO_RESOURCES &&
		...
		att->conn_context.prev_eatt_conn_req_result = 0;
		att->conn_context.prev_eatt_conn_req_missing_chans = 0;
...
static void ecred_connect_req_cb(struct bt_conn *conn, uint16_t result, uint16_t psm)
{
	...
	att->conn_context.prev_eatt_conn_rsp_result = result;
				 ...
				 uint16_t psm)
{
	...
	att->conn_context.prev_eatt_conn_req_result = result;
	att->conn_context.prev_eatt_conn_req_missing_chans =

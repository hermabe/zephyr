...
static void test_host_long_adv_recv(void)
{
	...
	generate_sequence(&report_a_combined.data[0], report_a_combined.length, 'A', 'Z');
	...
	ztest_returns_value(get_expected_data, &report_c.data[0]);
	...
	ztest_returns_value(get_expected_data, &report_b_2.data[0]);
	...
	ztest_returns_value(get_expected_data, &report_a_combined.data[0]);
	ztest_returns_value(get_expected_length, report_a_combined.length); // Expect only a
	ztest_returns_value(get_expected_data, &report_b_2.data[0]);
	ztest_returns_value(get_expected_length, report_b_2.length); // Expect b, INCOMPLETE REPORT
	send_adv_report(&report_a_1);
	...
	// packets and discards any further reports
	ztest_returns_value(get_expected_data, &report_a_1_repeated.data[0]);
	ztest_returns_value(get_expected_length, CONFIG_BT_EXT_SCAN_BUF_SIZE);

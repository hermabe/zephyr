...
static void test_peripheral_main(void)
{
	...
	/* Noop, central does the connecting */
...
static void test_central_main(void)
{
	...
	/* More than Peripheral CONFIG_BT_EATT_MAX (1) */
	/* Less than Central CONFIG_BT_EATT_MAX/2 (2.5) */
	size_t to_connect = 2;

	/* Will be rejected */
	err = bt_eatt_connect(default_conn, to_connect);
	if (err) {
	...
	num_enhanced = bt_eatt_count(default_conn);
	printk("%d enhanced bearers connected\n", num_enhanced);
	if (num_enhanced != 1) { /* Peripheral CONFIG_BT_EATT_MAX */
		FAIL("Expected %d enhanced bearers, got %d\n", CONFIG_BT_EATT_MAX, num_enhanced);
	}

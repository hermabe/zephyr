...
static bool volatile all_attributes_found;
static int volatile num_eatt_chans;
#define N_EATT_CHANNELS CONFIG_BT_EATT_MAX
...
static uint16_t char_1_attr_handle;
static uint16_t char_2_attr_handle;
			   ...
			   uint16_t len, uint16_t offset)
{
	...
	printk("%s bt_gatt_attr_read returned %d\n", __func__, err);
...
struct bt_gatt_attr gatt_attributes[] = {
	...
	BT_GATT_CHARACTERISTIC(&uuid_char_1.uuid, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
			       BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, read_char_1, NULL,
			       char_1_data),
	BT_GATT_CHARACTERISTIC(&uuid_char_2.uuid, BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE,
			       BT_GATT_PERM_READ | BT_GATT_PERM_WRITE, read_char_2, NULL,
			       char_2_data)
};
...
{
	if (att_err) {
		FAIL("MTU exchange failed (att_err %d)\n", att_err);
	}
}
				    ...
				    struct bt_gatt_discover_params *params)
{
	...
	discover_params_2.chan_option = BT_ATT_CHAN_ANY;
...
{
	static uint16_t gatt_read_cb_counter;

	printk("%s: read data: %d, length: %d, err: 0x%X\n", __func__, gatt_read_cb_counter++,
	       length, err);
...
static void test_central_main(void)
{
	...
	printk("Reading with flag BT_ATT_CHAN_UNENHANCED\n");
	do_reads(BT_ATT_CHAN_UNENHANCED);
	printk("Reads done\n");

	printk("Reading with flag BT_ATT_CHAN_ENHANCED\n");
	do_reads(BT_ATT_CHAN_ENHANCED);
	printk("Reads done\n");

	printk("Reading with flag BT_ATT_CHAN_ANY\n");
	do_reads(BT_ATT_CHAN_ANY);
	printk("Reads done\n");
...
static const struct bst_test_instance test_def[] = { { .test_id = "peripheral_gatt",
						       .test_descr = "Peripheral EATT",
						       .test_post_init_f = test_init,
						       .test_tick_f = test_tick,
						       .test_main_f = test_peripheral_main },
						     { .test_id = "central_gatt",
...
struct bst_test_list *test_main_gatt_install(struct bst_test_list *tests)
{

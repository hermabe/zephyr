...
static void do_reads()
{
	...
	struct bt_att_chan *enhanced = bt_eatt_get_channel(default_conn);
	struct bt_att_chan *unenhanced = att_get_fixed_chan(default_conn);

	struct bt_att_req *req_1 = bt_gatt_create_read_req(&read_params_1);
	struct bt_att_req *req_2 = bt_gatt_create_read_req(&read_params_2);
	err = bt_att_chan_req_send(enhanced, req_1);
	if (err) {
		FAIL("Gatt Read failed (err %d)\n", err);
	}

	err = bt_att_chan_req_send(unenhanced, req_2);
	if (err) {
		FAIL("Gatt Read failed (err %d)\n", err);
	}
